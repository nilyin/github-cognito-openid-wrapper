...
export class GitHubCognitoOpenIdWrapper extends cdk.Stack {
  constructor(scope: Construct, id: string, props: GitHubCognitoOpenIdWrapperProps) {
    super(scope, id, props);

    const githubAppClientSecret = secretsmanager.Secret.fromSecretAttributes(this, "githubAppClientSecret", {
    ...
    wellKnownResource.addResource("jwks.json").addMethod("GET", new apigateway.LambdaIntegration(jwks));

    api.root.addResource("authorize").addMethod("GET", new apigateway.LambdaIntegration(authorize));
